    ...
    "imagenet": {
        ...
        "id": {
            "train": {
                ...
                "weiper_kldiv": "benchmark_imglist/imagenet/train_imagenet_subsample.txt",
            },
            "val": {
                "data_dir": "images_largescale/",
...
def get_id_ood_dataloader(
    id_name, data_root, preprocessor, postprocessor_name, **loader_kwargs
):
    ...
    for split in data_info["id"].keys():
        dataset = ImglistDataset(
            ...
            imglist_pth=(
                os.path.join(data_root, data_info["id"][split]["weiper_kldiv"])
                if (
                    postprocessor_name == "weiper_kldiv"
                    and split == "train"
                    and id_name == "imagenet"
                )
                else os.path.join(data_root, data_info["id"][split]["imglist_path"])
            ),
